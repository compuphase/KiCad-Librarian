...
AppFrame::AppFrame( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxFrame( parent, id, title, pos, size, style )
{
	...
	m_toolZoomIn = m_toolBar->AddTool( IDT_ZOOMIN, wxT("Zoom in"), wxBitmap( ZoomIn_xpm ), wxNullBitmap, wxITEM_NORMAL, wxT("Zoom in (Ctrl +)."), wxEmptyString, NULL );

	m_toolZoomOut = m_toolBar->AddTool( IDT_ZOOMOUT, wxT("Zoom out"), wxBitmap( ZoomOut_xpm ), wxNullBitmap, wxITEM_NORMAL, wxT("Zoom out (Ctrl -)."), wxEmptyString, NULL );

	m_tool3DView = m_toolBar->AddTool( IDT_3DVIEW, wxT("tool"), wxBitmap( View3D_xpm ), wxNullBitmap, wxITEM_CHECK, wxEmptyString, wxEmptyString, NULL );

	m_toolMeasure = m_toolBar->AddTool( IDT_MEASUREMENTS, wxT("Measure"), wxBitmap( Measure_xpm ), wxNullBitmap, wxITEM_CHECK, wxT("Show pitch and pad measurements."), wxEmptyString, NULL );

	m_toolDetailsPanel = m_toolBar->AddTool( IDT_DETAILSPANEL, wxT("Details panel"), wxBitmap( Information_xpm ), wxNullBitmap, wxITEM_CHECK, wxT("Show/hide the side panel with parameters."), wxEmptyString, NULL );

	m_toolLeftFootprint = m_toolBar->AddTool( IDT_LEFTFOOTPRINT, wxT("Left"), wxBitmap( ArrowLeft_xpm ), wxNullBitmap, wxITEM_CHECK, wxT("Show left footprint."), wxEmptyString, NULL );

	m_toolRightFootprint = m_toolBar->AddTool( IDT_RIGHTFOOTPRINT, wxT("Right"), wxBitmap( ArrowRight_xpm ), wxNullBitmap, wxITEM_CHECK, wxT("Show right footprint."), wxEmptyString, NULL );
	...
	m_txtAlias->Hide();
	m_txtAlias->SetToolTip( wxT("Other names for the symbol, separated with spaces.") );
	...
	wxString m_choicePadShapeChoices[] = { wxT("(varies)"), wxT("Rectangular"), wxT("Round"), wxT("Round + square"), wxT("Obround"), wxT("Trapezoid") };
...
DlgRemoteLink::DlgRemoteLink( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxDialog( parent, id, title, pos, size, style )
{
	...
	m_lblHelp = new wxStaticText( this, wxID_ANY, wxT("Fill in the above fields. If you do not have an account yet, click on the button \"Sign up\"."), wxDefaultPosition, wxDefaultSize, 0|wxSTATIC_BORDER );
	m_lblHelp->Wrap( 300 );
...
DlgRemoteSignUp::DlgRemoteSignUp( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxDialog( parent, id, title, pos, size, style )
{
	...
	m_lblHelp = new wxStaticText( this, wxID_ANY, wxT("Fill in the above fields. You will receive a password by email."), wxDefaultPosition, wxDefaultSize, 0|wxSTATIC_BORDER );
	m_lblHelp->Wrap( 300 );
	m_lblHelp->SetMinSize( wxSize( 300,-1 ) );
